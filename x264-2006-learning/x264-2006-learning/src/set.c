...
void x264_cqm_init( x264_t *h )
{
    ...
    for( q = 0; q < 6; q++ )
    {
        for( i_list = 0; i_list < 4; i_list++ )
            for( i = 0; i < 16; i++ )
            {
                h->dequant4_mf[i_list][q][0][i] = def_dequant4[q][i] * 16;//h->pps->scaling_list[i_list][i];
                h->  quant4_mf[i_list][q][0][i] = def_quant4[q][i] * 16 /16;// h->pps->scaling_list[i_list][i];
            }
        for( i_list = 0; i_list < 2; i_list++ )
            for( i = 0; i < 64; i++ )
            {
                h->dequant8_mf[i_list][q][0][i] = def_dequant8[q][i] * 16;//h->pps->scaling_list[4+i_list][i];
                h->  quant8_mf[i_list][q][0][i] = def_quant8[q][i] * 16 /16;// h->pps->scaling_list[4+i_list][i];
            }
    }
    for( q = 0; q < 52; q++ )
    {
