...
void    x264_param_default( x264_param_t *param )
{
    ...
    param->rc.i_aq_mode = X264_AQ_NONE ;//X264_AQ_VARIANCE;
...
{
    UINT8 *dst = p_data;
    UINT8 *src = nal->p_payload;
    UINT8 *end = &nal->p_payload[nal->i_payload];
...
int x264_nal_decode( x264_nal_t *nal, void *p_data, int i_data )
{
    ...
    nal->i_payload = dst - (UINT8*)p_data;
    return 0;
}
...
void *x264_malloc( int i_size )
{
    ...
    UINT8 * buf;
    UINT8 * align_buf;
    buf = (UINT8 *) malloc( i_size + 15 + sizeof( void ** ) +
              sizeof( int ) );
...
void *x264_realloc( void *p, int i_size )
{
    ...
    if( p )
    {
        i_old_size = *( (int*) ( (UINT8*) p ) - sizeof( void ** ) -
                         sizeof( int ) );
    }
