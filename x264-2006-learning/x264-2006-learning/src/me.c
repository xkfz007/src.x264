...
void x264_me_search_ref( x264_t *h, x264_me_t *m, int (*mvc)[2], int i_mvc, int *p_halfpel_thresh )
{
    ...
    const INT16 *p_cost_mvx = m->p_cost_mv - m->mvp[0];
    const INT16 *p_cost_mvy = m->p_cost_mv - m->mvp[1];
    ...
    case X264_ME_ESA:
        {
            ...
            static UINT8 zero[16*16] = {0,};
            const int enc_dc = h->pixf.sad[i_pixel]( m->p_fenc[0], FENC_STRIDE, zero, 16 );
            const UINT16 *integral_base = &m->integral[ -1 - 1*stride ];
            ...
            if( h->pixf.sad_pde[i_pixel] )
            {
                for( my = min_y; my <= max_y; my++ )
                    for( mx = min_x; mx <= max_x; mx++ )
                    {
                        const UINT16 *integral = &integral_base[ mx + my * stride ];
                        const UINT16 ref_dc = integral[  0 ] + integral[ dh + dw ]
...
{ \
    int stride = 16; \
    UINT8 *src = h->mc.get_ref( m->p_fref, m->i_stride[0], pix, &stride, mx, my, bw, bh ); \
...
static void refine_subpel( x264_t *h, x264_me_t *m, int hpel_iters, int qpel_iters, int *p_halfpel_thresh, int b_refine_qpel )
{
    ...
    DECLARE_ALIGNED( UINT8, pix[16*16], 16 );
    int omx, omy;
    int i;
...
int x264_me_refine_bidir( x264_t *h, x264_me_t *m0, x264_me_t *m1, int i_weight )
{
    ...
    const int bs = bw*bh;
    const INT16 *p_cost_m0x = m0->p_cost_mv - x264_clip3( m0->mvp[0], h->mb.mv_min_spel[0], h->mb.mv_max_spel[0] );
    const INT16 *p_cost_m0y = m0->p_cost_mv - x264_clip3( m0->mvp[1], h->mb.mv_min_spel[0], h->mb.mv_max_spel[0] );
    const INT16 *p_cost_m1x = m1->p_cost_mv - x264_clip3( m1->mvp[0], h->mb.mv_min_spel[0], h->mb.mv_max_spel[0] );
    const INT16 *p_cost_m1y = m1->p_cost_mv - x264_clip3( m1->mvp[1], h->mb.mv_min_spel[0], h->mb.mv_max_spel[0] );
    DECLARE_ALIGNED( UINT8, pix0[9][16*16], 16 );
    DECLARE_ALIGNED( UINT8, pix1[9][16*16], 16 );
    DECLARE_ALIGNED( UINT8, pix[16*16], 16 );
