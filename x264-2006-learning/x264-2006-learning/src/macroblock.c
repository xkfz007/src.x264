...
void x264_mb_predict_mv( x264_t *h, int i_list, int idx, int i_width, int mvp[2] )
{
    ...
    INT16 *mv_a  = h->mb.cache.mv[i_list][i8 - 1];
    int     i_refb = h->mb.cache.ref[i_list][i8 - 8];
    INT16 *mv_b  = h->mb.cache.mv[i_list][i8 - 8];
    int     i_refc = h->mb.cache.ref[i_list][i8 - 8 + i_width ];
    INT16 *mv_c  = h->mb.cache.mv[i_list][i8 - 8 + i_width];
...
void x264_mb_predict_mv_16x16( x264_t *h, int i_list, int i_ref, int mvp[2] )
{
    ...
    INT16 *mv_a  = h->mb.cache.mv[i_list][X264_SCAN8_0 - 1];
    int     i_refb = h->mb.cache.ref[i_list][X264_SCAN8_0 - 8];
    INT16 *mv_b  = h->mb.cache.mv[i_list][X264_SCAN8_0 - 8];
    int     i_refc = h->mb.cache.ref[i_list][X264_SCAN8_0 - 8 + 4];
    INT16 *mv_c  = h->mb.cache.mv[i_list][X264_SCAN8_0 - 8 + 4];
...
void x264_mb_predict_mv_pskip( x264_t *h, int mv[2] )
{
    ...
    INT16 *mv_a  = h->mb.cache.mv[0][X264_SCAN8_0 - 1];
    INT16 *mv_b  = h->mb.cache.mv[0][X264_SCAN8_0 - 8];
...
static int x264_mb_predict_mv_direct16x16_temporal( x264_t *h )
{
    ...
    for( i8 = 0; i8 < 4; i8++ )
    {
        ...
        if( i_ref >= 0 )
        {
            ...
            if( b8x8 )
            {
                const INT16 *mv_col = h->fref1[0]->mv[0][ i_mb_4x4 + 3*x8 + 3*y8 * h->mb.i_b4_stride];
                int mv_l0[2];
            ...
            else
            {
                for( i4 = 0; i4 < 4; i4++ )
                {
                    ...
                    const INT16 *mv_col = h->fref1[0]->mv[0][ i_mb_4x4 + x4 + y4 * h->mb.i_b4_stride ];
                    int mv_l0[2];
                    mv_l0[0] = ( dist_scale_factor * mv_col[0] + 128 ) >> 8;
                    mv_l0[1] = ( dist_scale_factor * mv_col[1] + 128 ) >> 8;
...
static int x264_mb_predict_mv_direct16x16_spatial( x264_t *h )
{
    ...
    int i8, i4;
    int b8x8;
    const INT8 *l1ref0 = &h->fref1[0]->ref[0][ h->mb.i_b8_xy ];
    const INT8 *l1ref1 = &h->fref1[0]->ref[1][ h->mb.i_b8_xy ];
    const INT16 (*l1mv0)[2] = (const INT16 (*)[2]) &h->fref1[0]->mv[0][ h->mb.i_b4_xy ];
    const INT16 (*l1mv1)[2] = (const INT16 (*)[2]) &h->fref1[0]->mv[1][ h->mb.i_b4_xy ];
    const int type_col = h->fref1[0]->mb_type[ h->mb.i_mb_xy ];
    ...
    for( i8=0; i8<4; i8++ )
    {
        ...
        {
            const INT16 (*l1mv)[2] = (l1ref0[o8] == 0) ? l1mv0 : l1mv1;
            if( b8x8 )
            {
                const INT16 *mvcol = l1mv[3*x8 + 3*y8 * h->mb.i_b4_stride];
            ...
            else
            {
                for( i4=0; i4<4; i4++ )
                {
                    const int x4 = i4%2 + 2*x8;
                    const int y4 = i4/2 + 2*y8;
                    const INT16 *mvcol = l1mv[x4 + y4 * h->mb.i_b4_stride];
                    if( abs( mvcol[0] ) <= 1 && abs( mvcol[1] ) <= 1 )
                    {
                        if( ref[0] == 0 )
    ...
    if( b_changed != NULL && b_available )
    {
        ...
        if( IS_INTRA(type_col) || type_col == P_SKIP )
        {
            *b_changed = h->mb.cache.direct_ref[0][0] != h->mb.cache.ref[0][X264_SCAN8_0]
                      ...
                      || *(UINT32*)h->mb.cache.direct_mv[0][X264_SCAN8_0] != *(UINT32*)h->mb.cache.mv[0][X264_SCAN8_0]
                      || *(UINT32*)h->mb.cache.direct_mv[1][X264_SCAN8_0] != *(UINT32*)h->mb.cache.mv[1][X264_SCAN8_0];
        }
        else
        {
            int i, l;
...
void x264_mb_load_mv_direct8x8( x264_t *h, int idx )
{
    ...
    for( l = 0; l < 2; l++ )
    {
        *(UINT64*)h->mb.cache.mv[l][x264_scan8[idx*4]] =
        *(UINT64*)h->mb.cache.direct_mv[l][x264_scan8[idx*4]];
        *(UINT64*)h->mb.cache.mv[l][x264_scan8[idx*4]+8] =
        *(UINT64*)h->mb.cache.direct_mv[l][x264_scan8[idx*4]+8];
    }
}

/* This just improves encoder performance, it's not part of the spec */
void x264_mb_predict_mv_ref16x16( x264_t *h, int i_list, int i_ref, int mvc[9][2], int *i_mvc )
{
    INT16 (*mvr)[2] = h->mb.mvr[i_list][i_ref];
    int i = 0;
...
#define SET_MVP(mvp) { \
    ...
    if( i_ref == 0 && h->frames.b_have_lowres )
    {
        ...
        {
            INT16 (*lowres_mv)[2] = h->fenc->lowres_mvs[i_list][idx];
            if( lowres_mv[0][0] != 0x7fff )
            {
                 //mvc[i]  = (M32( lowres_mv[h->mb.i_mb_xy] )*2)&0xfffeffff;
				//*(int*)(mvc[i])=(*(int*)(lowres_mv[h->mb.i_mb_xy])*2)&0xfffeffff;
				mvc[i][0]=lowres_mv[h->mb.i_mb_xy][0]*2;
				mvc[i][1]=lowres_mv[h->mb.i_mb_xy][1]*2;
                i++;
            }
        }
    }
...
static inline void x264_mb_mc_01xywh( x264_t *h, int x, int y, int width, int height )
{
    ...
    DECLARE_ALIGNED( UINT8, tmp[16*16], 16 );
...
void x264_macroblock_cache_init( x264_t *h )
{
    ...
    h->mb.qp  = x264_malloc( i_mb_count * sizeof(INT8) );
    h->mb.cbp = x264_malloc( i_mb_count * sizeof(INT16) );
    h->mb.skipbp = x264_malloc( i_mb_count * sizeof(INT8) );
    h->mb.mb_transform_size = x264_malloc( i_mb_count * sizeof(INT8) );

    /* 0 -> 3 top(4), 4 -> 6 : left(3) */
    h->mb.intra4x4_pred_mode = x264_malloc( i_mb_count * 7 * sizeof( INT8 ) );

    /* all coeffs */
    h->mb.non_zero_count = x264_malloc( i_mb_count * 24 * sizeof( UINT8 ) );

    if( h->param.b_cabac )
    {
        h->mb.chroma_pred_mode = x264_malloc( i_mb_count * sizeof( INT8) );
        h->mb.mvd[0] = x264_malloc( 2*16 * i_mb_count * sizeof( INT16 ) );
        h->mb.mvd[1] = x264_malloc( 2*16 * i_mb_count * sizeof( INT16 ) );
    }
    ...
    for( i=0; i<2; i++ )
    {
        ...
        for( j=0; j < i_refs && j < 16; j++ )
            h->mb.mvr[i][j] = x264_malloc( 2 * i_mb_count * sizeof( INT16 ) );
    }

    /* init with not avaiable (for top right idx=7,15) */
    memset( h->mb.cache.ref[0], -2, X264_SCAN8_SIZE * sizeof( INT8 ) );
    memset( h->mb.cache.ref[1], -2, X264_SCAN8_SIZE * sizeof( INT8 ) );
}
...
void x264_macroblock_slice_init( x264_t *h )
{
    ...
    if( h->sh.i_type == SLICE_TYPE_P )
        memset( h->mb.cache.skip, 0, X264_SCAN8_SIZE * sizeof( INT8 ) );
}
...
void x264_macroblock_cache_load( x264_t *h, int i_mb_x, int i_mb_y )
{
    ...
    for( i = 0; i < 3; i++ )
    {
        ...
        const UINT8 *plane_fdec = &h->fdec->plane[i][ w * (i_mb_x + i_mb_y * i_stride) ];
        int   j;
