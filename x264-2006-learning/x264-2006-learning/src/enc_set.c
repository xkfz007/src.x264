...
static void transpose( UINT8 *buf, int w )
{
    int i, j;
    for( i = 0; i < w; i++ )
        for( j = 0; j < i; j++ )
            XCHG( UINT8, buf[w*i+j], buf[w*j+i] );
}
#if 0
static void scaling_list_write( bs_t *s, x264_pps_t *pps, int idx )
{
    ...
    const UINT8 *list = pps->scaling_list[idx];
    const UINT8 *def_list = (idx==CQM_4IC) ? pps->scaling_list[CQM_4IY]
    ...
    else
    {
        ...
        for( run = len; run > 1; run-- )
            if( list[zigzag[run-1]] != list[zigzag[run-2]] )
                break;
        if( run < len && len - run < bs_size_se( (INT8)-list[zigzag[run]] ) )
            run = len;

        for( j = 0; j < run; j++ )
            bs_write_se( s, (INT8)(list[zigzag[j]] - (j>0 ? list[zigzag[j-1]] : 8)) ); // delta

        if( run < len )
            bs_write_se( s, (INT8)-list[zigzag[run]] );
    }
}
#endif
...
#define CHECK( name, limit, val ) \
    ...
    if( h->param.i_fps_den > 0 )
        CHECK( "MB rate", l->mbps, (INT64)mbs * h->param.i_fps_num / h->param.i_fps_den );
