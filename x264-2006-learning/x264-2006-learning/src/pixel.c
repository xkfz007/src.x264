...
static int name( UINT8 *pix1, int i_stride_pix1,  \
...
INT64 x264_pixel_ssd_wxh( x264_pixel_function_t *pf, UINT8 *pix1, int i_pix1, UINT8 *pix2, int i_pix2, int i_width, int i_height )
{
    INT64 i_ssd = 0;
    int x, y;
 ...
 ****************************************************************************/
static int pixel_satd_wxh( UINT8 *pix1, int i_pix1, UINT8 *pix2, int i_pix2, int i_width, int i_height )
{
    INT16 tmp[4][4];
    INT16 diff[4][4];
    int x, y;
    ...
    for( y = 0; y < i_height; y += 4 )
    {
        for( x = 0; x < i_width; x += 4 )
        {
            ...
            pixel_sub_wxh( (INT16*)diff, 4, &pix1[x], i_pix1, &pix2[x], i_pix2 );
...
static inline int pixel_sa8d_wxh( UINT8 *pix1, int i_pix1, UINT8 *pix2, int i_pix2,
                                  int i_width, int i_height )
{
    ...
    for( y = 0; y < i_height; y += 8 )
    {
        ...
        {
            int i;
            pixel_sub_wxh( (INT16*)diff, 8, pix1+x, i_pix1, pix2+x, i_pix2 );
...
static int pixel_sa8d_##width##x##height( UINT8 *pix1, int i_stride_pix1, \
                 UINT8 *pix2, int i_stride_pix2 ) \
{ \
...
static UINT64 name( UINT8 *pix, int i_stride ) \
{                                             \
    UINT32 sum = 0, sqr = 0;                \
	int y,x;                                  \
    for(y = 0; y < h; y++ )              \
