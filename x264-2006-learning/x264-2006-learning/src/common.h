...
#define X264_VERSION "x264-20060316-2245" // no configure script for msvc
#ifdef _MSC_VER
...
#define x264_clip_uint8(x) (x&(~255) ? (-x)>>31 : x)
...
#define QP_BD_OFFSET (6*(X264_BIT_DEPTH-8))
...
struct x264_t
{
    ...
    struct
    {
        ...
        UINT8     *p_bitstream;   /* will hold data for all nal */
        bs_t        bs;
    } out;
    ...
    UINT32        nr_residual_sum[2][64];
    UINT32        nr_offset[2][64];
    UINT32        nr_count[2];
    ...
    struct
    {
        ...
        INT8  *type;                      /* mb type */
        INT8  *qp;                        /* mb qp */
        INT16 *cbp;                       /* mb cbp: 0x0?: luma, 0x?0: chroma, 0x100: luma dc, 0x0200 and 0x0400: chroma dc  (all set for PCM)*/
        INT8  (*intra4x4_pred_mode)[7];   /* intra4x4 pred mode. for non I4x4 set to I_PRED_4x4_DC(2) */
        UINT8 (*non_zero_count)[16+4+4];  /* nzc. for I_PCM set to 16 */
        INT8  *chroma_pred_mode;          /* chroma_pred_mode. cabac only. for non intra I_PRED_CHROMA_DC(0) */
        INT16 (*mv[2])[2];                /* mb mv. set to 0 for intra mb */
        INT16 (*mvd[2])[2];               /* mb mv difference with predict. set to 0 if intra. cabac only */
        INT8   *ref[2];                   /* mb ref. set to -1 if non used (intra or Lx only) */
        INT16 (*mvr[2][16])[2];           /* 16x16 mv for each possible ref */
        INT8  *skipbp;                    /* block pattern for SKIP or DIRECT (sub)mbs. B-frames + cabac only */
...
#define FDEC_STRIDE 32
            DECLARE_ALIGNED( UINT8, fenc_buf[24*FENC_STRIDE], 16 );
            DECLARE_ALIGNED( UINT8, fdec_buf[27*FDEC_STRIDE], 16 );
            ...
            UINT8 *p_fref[2][16][4+2]; /* last: lN, lH, lV, lHV, cU, cV */
            UINT16 *p_integral[2][16];
        ...
        struct
        {
            ...
            INT8  ref[2][X264_SCAN8_SIZE];

            /* 0 if non avaible */
            INT16 mv[2][X264_SCAN8_SIZE][2];
            INT16 mvd[2][X264_SCAN8_SIZE][2];

            /* 1 if SKIP or DIRECT. set only for B-frames + CABAC */
            INT8  skip[X264_SCAN8_SIZE];

            INT16 direct_mv[2][X264_SCAN8_SIZE][2];
            INT8  direct_ref[2][X264_SCAN8_SIZE];
    ...
    struct
    {
        ...
        /* per slice info */
        int     i_slice_count[3];
        INT64 i_slice_size[3];
        double  f_frame_qp[3];
        /* */
        INT64 i_sqe_global[3];
        float   f_psnr_average[3];
        float   f_psnr_mean_y[3];
        float   f_psnr_mean_u[3];
        float   f_psnr_mean_v[3];
        float   f_ssim_average[3];
        float   f_ssim_mean_y[3];
        float   f_ssim_mean_u[3];
        float   f_ssim_mean_v[3];
        /* */
        INT64 i_mb_count[3][19];
        INT64 i_mb_count_8x8dct[2];
        INT64 i_mb_count_size[2][7];
        INT64 i_mb_count_ref[2][16];
        /* */
        int     i_direct_score[2];
