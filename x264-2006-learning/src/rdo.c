...
typedef struct {
    ...
    UINT8 cabac_state[10]; //just the contexts relevant to coding abs_level_m1
} trellis_node_t;
...
static void quant_trellis_cabac( x264_t *h, INT16 *dct,
                                 ...
                                 int i_ctxBlockCat, int i_qbits, int i_lambda2, int b_ac, int i_coefs )
{
    ...
    UINT8 cabac_state_sig[64];
    UINT8 cabac_state_last[64];
    ...
    if( i_coefs == 64 )
    {
        const UINT8 *ctx_sig  = &h->cabac.state[ significant_coeff_flag_offset[i_ctxBlockCat] ];
        const UINT8 *ctx_last = &h->cabac.state[ last_coeff_flag_offset[i_ctxBlockCat] ];
        for( i = 0; i < 63; i++ )
        {
...
#define SET_LEVEL(n,l) \
        ...
        for( abs_level = q; abs_level >= q-1; abs_level-- )
        {
            ...
            UINT64 ssd = (INT64)d*d * coef_weight[i];
            ...
            for( j = 0; j < 8; j++ )
            {
                ...
                if( abs_level || node_ctx )
                {
                    ...
                    if( abs_level )
                    {
                        ...
                        if( i_prefix > 0 )
                        {
                            UINT8 *ctx = &n.cabac_state[coeff_abs_levelgt1_ctx[node_ctx]];
                    ...
                    n.score += (UINT64)f8_bits * i_lambda2 >> ( CABAC_SIZE_BITS - LAMBDA_BITS );
                }
                             ...
                             int i_qp, int i_ctxBlockCat, int b_intra )
{
    ...
    quant_trellis_cabac( h, (INT16*)dct,
