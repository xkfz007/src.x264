...
static INT64 i_mtime_encode_frame = 0;
static INT64 i_mtime_analyse = 0;
static INT64 i_mtime_encode = 0;
static INT64 i_mtime_write = 0;
static INT64 i_mtime_filter = 0;
#define TIMER_START( d ) \
    { \
        INT64 d##start = x264_mdate();
 ...
 *
 ****************************************************************************/
static float x264_psnr( INT64 i_sqe, INT64 i_size )
{
...
static float x264_ssim( UINT8* pic_r, int stride_r,
					   UINT8* pic_i, int stride_i,
...
static void x264_frame_dump( x264_t *h, x264_frame_t *fr )
{
    FILE * f = fopen( h->param.psz_dump_yuv, "r+b" );
    int i, y;
	int frame_size=h->param.i_width*h->param.i_height*sizeof(UINT8)*3/2;

    fseek( f,frame_size*fr->i_frame , SEEK_SET );
 ...
 //   fprintf(stderr,"param->i_slice_max_mbs=%d\n",param->i_slice_max_mbs);
  //  fprintf(stderr,"param->i_slice_count=%d\n",param->i_slice_count);
	...
	*/
    ...
    fprintf(stderr,"param->rc.f_rf_constant =%d\n",param->rc.i_rf_constant);
	...
	fprintf(stderr,"param->rc.b_brate=%d\n",param->rc.b_brate);
	/*
...
//    h->frames.i_delay = h->param.i_bframe;
    ...
    h->frames.b_have_lowres = !h->param.rc.b_stat_read
        && ( h->param.rc.b_cbr || h->param.rc.i_rf_constant 
		|| h->param.i_bframe_adaptive || h->param.rc.b_mb_tree
		);

	//h->frames.unused=(x264_frame_t**)malloc((h->frames.i_delay + 3) * sizeof(x264_frame_t *));
	//h->frames.current=(x264_frame_t**)malloc((h->frames.i_delay + 3) * sizeof(x264_frame_t *));
	//h->frames.next=(x264_frame_t**)malloc((h->frames.i_delay + 3) * sizeof(x264_frame_t *));
	h->frames.unused=x264_malloc((h->frames.i_delay + 3) * sizeof(x264_frame_t *));
	h->frames.current=x264_malloc((h->frames.i_delay + 3) * sizeof(x264_frame_t *));
	h->frames.next=x264_malloc((h->frames.i_delay + 3) * sizeof(x264_frame_t *));

    for( i = 0; i < h->frames.i_delay + 3; i++ )
    {
        h->frames.current[i] = NULL;
        h->frames.next[i]    = NULL;
        h->frames.unused[i]  = NULL;
    }
    for( i = 0; i <  h->frames.i_delay+1; i++ )
    {
...
//    x264_csp_init( h->param.cpu, h->param.i_csp, &h->csp );
    ...
    if( x264_ratecontrol_new( h ) < 0 )
        return NULL;
	if(strlen(h->param.psz_dump_yuv)>0){
...
static inline void x264_reference_build_list( x264_t *h, int i_poc, int i_slice_type )
{
	...
	fprintf(stderr,"L0:");
	for(i=0;i<h->i_ref0;i++){
		fprintf(stderr,"%d ",h->fref0[i]->i_poc);
	}

	fprintf(stderr,"  L1:");
	for(i=0;i<h->i_ref1;i++){
		fprintf(stderr,"%d ",h->fref1[i]->i_poc);
	}
	fprintf(stderr,"\n");
	fflush(stderr);
#endif
...
static inline void x264_reference_update( x264_t *h )
{
    ...
    for( i = 0; i < 4; i++)
        XCHG( UINT8*, h->fdec->lowres[i], h->fenc->lowres[i] );
...
#if 0
	fprintf(stderr,"DPB:");
	for(i=0;i<h->frames.i_max_dpb-1;i++){
		fprintf(stderr,"%d ",h->frames.reference[i]->i_poc);
	}
	fprintf(stderr,"\n");
	fflush(stderr);
                             ...
                             x264_picture_t *pic_out )
{
    ...
    char psz_psnr_msg[80];
    char psz_ssim_msg[80];
	char psz_brate_msg[50];
	char *stype=NULL;
    ...
    *pp_nal = NULL;
    ...
    if( pic_in != NULL )
    {
		...
		if(h->param.rc.i_aq_mode)
            x264_adaptive_quant_frame(h, fenc, 0);//pic_in->prop.quant_offsets );
...
do_encode:
	...
	if(h->param.rc.b_mb_tree&&IS_X264_TYPE_I(h->fenc->i_type))
	{
...
#if 0
	{
		...
		for(y=0;y<h->sps->i_mb_height;y++)
		{
			for(x=0;x<h->sps->i_mb_width;x++){
				int mb_index=x+y*h->mb.i_mb_stride;
				printf("%5.2f ",h->fenc->f_qp_offset[mb_index]);
			}
			printf("\n");
		}
		fflush(stdout);
        ...
        && h->param.i_scenecut_threshold > 0 )
    {
        ...
        INT64 i_inter_cost = h->stat.frame.i_inter_cost;
        INT64 i_intra_cost = h->stat.frame.i_intra_cost;
    ...
    *pp_nal = h->out.nal;
    ...
    h->stat.f_frame_qp[i_slice_type] += frame_psnr->f_qp_avg_aq;
    ...
    if( h->param.analyse.b_psnr )
    {
        ...
        sprintf( psz_psnr_msg,  " PSNR Y:%2.2f U:%2.2f V:%2.2f",
    ...
    if( h->param.analyse.b_ssim )
	{
        ...
        ssimy= x264_ssim(frame_psnr->plane[0],frame_psnr->i_stride[0],h->fenc->plane[0],h->fenc->i_stride[0],h->param.i_width,h->param.i_height );
        ssimu= x264_ssim(frame_psnr->plane[1],frame_psnr->i_stride[1],h->fenc->plane[1],h->fenc->i_stride[1],h->param.i_width/2,h->param.i_height/2 );
        ssimv= x264_ssim(frame_psnr->plane[2],frame_psnr->i_stride[2],h->fenc->plane[2],h->fenc->i_stride[2],h->param.i_width/2,h->param.i_height/2 );

        h->stat.f_ssim_average[i_slice_type]+=(0.8*ssimy+0.1*ssimu+0.1*ssimv);
        h->stat.f_ssim_mean_y[i_slice_type] +=ssimy; 
        h->stat.f_ssim_mean_u[i_slice_type] +=ssimu;
        h->stat.f_ssim_mean_v[i_slice_type] +=ssimv; 
        sprintf( psz_ssim_msg,  " SSIM Y:%.4f U:%.4f V:%.4f", ssimy,ssimu,ssimv);
        psz_ssim_msg[79] = '\0';
	}
    else
    {
	...
	if(h->param.rc.b_brate){
		...
		INT64 siz=h->stat.i_slice_size[SLICE_TYPE_I] + 
			h->stat.i_slice_size[SLICE_TYPE_P] + 
			h->stat.i_slice_size[SLICE_TYPE_B];
		
        double f_bitrate = fps * siz*1.0 / i_count / 125.0;
        sprintf( psz_brate_msg,  " Brate:%.2f", f_bitrate);
		psz_brate_msg[49]='\0';
	}
	else
		psz_brate_msg[0]='\0';
    ...
    x264_log( h, X264_LOG_DEBUG,
                  "frame=%4d QP=%.2f NAL=%d Slice:%c(%s) Poc:%-3d I:%-4d P:%-4d SKIP:%-4d size=%d bytes%s%s%s\n",
              h->i_frame - 1,
              //i_global_qp,
			  frame_psnr->f_qp_avg_aq,
              i_nal_ref_idc,
              ...
              i_frame_size,
              psz_psnr_msg,psz_ssim_msg,psz_brate_msg );
...
{
#ifdef DEBUG_BENCHMARK
    INT64 i_mtime_total = i_mtime_analyse + i_mtime_encode + i_mtime_write + i_mtime_filter + 1;
#endif
    INT64 i_yuv_size = 3 * h->param.i_width * h->param.i_height / 2;
    int i;
    ...
    for( i=0; i<3; i++ )
    {
        ...
        if( h->stat.i_slice_count[i_slice] > 0 )
        {
			...
			x264_log( h, X264_LOG_INFO,
                          ...
                          slice_name[i_slice],
                          i_count,
                          (double)h->stat.f_frame_qp[i_slice] / i_count,
                          (double)h->stat.i_slice_size[i_slice] / i_count );
            if( h->param.analyse.b_psnr )
            {
                x264_log( h, X264_LOG_CONT,
                          "PSNR Mean Y:%5.2f U:%5.2f V:%5.2f Avg:%5.2f Global:%5.2f ",
            ...
            {
                x264_log( h, X264_LOG_CONT,
                          "SSIM Mean Y:%.4f U:%.4f V:%.4f Avg:%.4f ",
                          h->stat.f_ssim_mean_y[i_slice] / i_count, h->stat.f_ssim_mean_u[i_slice] / i_count, h->stat.f_ssim_mean_v[i_slice] / i_count,
                          h->stat.f_ssim_average[i_slice] / i_count
                           );
            }
			x264_log( h, X264_LOG_CONT,"\n");
    ...
    {
        const INT64 *i_mb_count = h->stat.i_mb_count[SLICE_TYPE_I];
    ...
    {
        const INT64 *i_mb_count = h->stat.i_mb_count[SLICE_TYPE_P];
        const INT64 *i_mb_size = h->stat.i_mb_count_size[SLICE_TYPE_P];
    ...
    {
        const INT64 *i_mb_count = h->stat.i_mb_count[SLICE_TYPE_B];
        const INT64 *i_mb_size = h->stat.i_mb_count_size[SLICE_TYPE_B];
...
#define SUM3b(p,o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
        ...
        if( h->param.analyse.b_transform_8x8 )
        {
            INT64 i_i8x8 = SUM3b( h->stat.i_mb_count, I_8x8 );
            INT64 i_intra = i_i8x8 + SUM3b( h->stat.i_mb_count, I_4x4 )
		...
		x264_log( h, X264_LOG_INFO, "kb/s:%.1f ", f_bitrate );

        if( h->param.analyse.b_psnr )
            x264_log( h, X264_LOG_CONT,
                      "PSNR Mean Y:%6.3f U:%6.3f V:%6.3f Avg:%6.3f Global:%6.3f ",
        ...
        if( h->param.analyse.b_ssim )
            x264_log( h, X264_LOG_CONT,
                      ...
                      SUM3( h->stat.f_ssim_mean_u ) / i_count,
                      SUM3( h->stat.f_ssim_mean_v ) / i_count,
                      SUM3( h->stat.f_ssim_average ) / i_count
                      );
			x264_log( h, X264_LOG_CONT,"\n");
    }
	...
	x264_free(h->frames.unused);
	x264_free(h->frames.next);
