...
#define X264_ME_DIA                  0 //motion estimation菱形搜索
#define X264_ME_HEX                  1 //六边形搜索，默认值
#define X264_ME_UMH                  2 //非对称十字形六边形格点搜索算法
...
typedef struct x264_param_t
{
    ...
    /* Video Properties */
    int         i_width;	   	// Picture width in pixels 
    int         i_height;		// Picture height in pixels 
    int         i_csp;         /* CSP(color space) of encoded bitstream */
    int         i_level_idc;
...
typedef struct
{
    ...
    int     b_keyframe;
    /* In: user pts, 
       Out: pts of encoded picture (user)
    */
    int64_t i_pts;
