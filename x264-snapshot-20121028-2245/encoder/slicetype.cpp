...
//extern struct x264_mb_analysis_t;
...
//            weights[plane].i_scale = x264_clip3( round( guess_scale * 64 ), 0, 255 ); //TODO
            weights[plane].i_scale = x264_clip3( floor( guess_scale * 64 + 0.5), 0, 255 ); //TODO
		...
		{
//            x264_weight_get_h264( round( guess_scale * 128 ), 0, &weights[plane] );//TODO
			x264_weight_get_h264( floor( guess_scale * 128 + 0.5), 0, &weights[plane] );//TODO
		}
                                      ...
                                      int b_intra_penalty )
{
    ...
    else
    {
        ...
        output_inter[0] = (int*)h->scratch_buffer2;
        ...
        if( h->param.i_lookahead_threads > 1 )
        {
            ...
            for( int i = 0; i < h->param.i_lookahead_threads; i++ )
            {
				...
				x264_slicetype_slice_t temp_struct = {t, a, frames, p0, p1, b, dist_scale_factor, do_search, w,output_inter[i], output_intra[i]};

				s[i] = temp_struct;


//                s[i] = (x264_slicetype_slice_t){ t, a, frames, p0, p1, b, dist_scale_factor, do_search, w,
//                                                 output_inter[i], output_intra[i] }; //TODO
                ...
                x264_threadpool_run( h->lookaheadpool, (void *(__cdecl *)(void *))x264_slicetype_slice_cost, &s[i] );
            }
            for( int i = 0; i < h->param.i_lookahead_threads; i++ )
...
{
 //   int fps_factor = round( CLIP_DURATION(average_duration) / CLIP_DURATION(frame->f_duration) * 256 );//TODO
    int fps_factor = floor( CLIP_DURATION(average_duration) / CLIP_DURATION(frame->f_duration) * 256 +0.5);
...
int x264_rc_analyse_slice( x264_t *h )
{
    ...
    assert( cost >= 0 ); //TODO , Reason Unknown
//	printf("%d\n",cost);
