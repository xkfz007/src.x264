...
{
    int csp = param->i_csp & X264_CSP_MASK; // Color space

    sps->i_id = i_id;
    sps->i_mb_width = ( param->i_width + 15 ) / 16;
    sps->i_mb_height= ( param->i_height + 15 ) / 16;
	/*Refer to the comment for more info*/
...
int x264_sei_version_write( x264_t *h, bs_t *s )
{
    ...
    if( !opts )
        return -1;
	void * temp;
    CHECKED_MALLOC(temp , 200 + strlen( opts ) );
	payload = (char*)temp;
...
#define CHECK( name, limit, val ) \
    if( (val) > (limit) ) \
        ERROR( name " (%I64d) > level limit (%d)\n", (int64_t)(val), (limit) );//TODO
