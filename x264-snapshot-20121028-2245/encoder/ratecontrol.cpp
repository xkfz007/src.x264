...
static int x264_macroblock_tree_rescale_init( x264_t *h, x264_ratecontrol_t *rc )
{
    ...
    void *temp;
    CHECKED_MALLOC(temp , rc->mbtree.src_mb_count * sizeof(uint16_t) );
	rc->mbtree.qp_buffer[0] = (uint16_t *)temp;
    if( h->param.i_bframe_pyramid && h->param.rc.b_stat_read )
	{

        CHECKED_MALLOC(temp , rc->mbtree.src_mb_count * sizeof(uint16_t) );
		rc->mbtree.qp_buffer[1] = (uint16_t *)temp;
	}
    ...
    CHECKED_MALLOC(temp , srcdimi[0] * srcdimi[1] * sizeof(float) );
	rc->mbtree.scale_buffer[0] = (float *)temp;
    CHECKED_MALLOC(temp , dstdimi[0] * srcdimi[1] * sizeof(float) );
    rc->mbtree.scale_buffer[1] = (float *)temp;
    ...
    for( int i = 0; i < 2; i++ )
    {
        ...
        CHECKED_MALLOC(temp, rc->mbtree.filtersize[i] * dstdimi[i] * sizeof(float) );
		rc->mbtree.coeffs[i] = (float *)temp;
        CHECKED_MALLOC(temp , dstdimi[i] * sizeof(int) );
        rc->mbtree.pos[i] = (int *)temp;
...
int x264_ratecontrol_new( x264_t *h )
{
    ...
    CHECKED_MALLOCZERO(temp , h->param.i_threads * sizeof(x264_ratecontrol_t) );
	h->rc = (x264_ratecontrol_t *)temp;
    rc = h->rc;

    rc->b_abr = h->param.rc.i_rc_method != X264_RC_CQP && !h->param.rc.b_stat_read; //??
    rc->b_2pass = h->param.rc.i_rc_method == X264_RC_ABR && h->param.rc.b_stat_read;//??

    /* FIXME: use integers,TODO */
    ...
    rc->bitrate = h->param.rc.i_bitrate * 1000.;		// Target bitrate
    rc->rate_tolerance = h->param.rc.f_rate_tolerance;	// Err tolerate
    rc->nmb = h->mb.i_mb_count;							// MB count to be encoded
...
//    void * temp;
    CHECKED_MALLOC(temp , 5 * sizeof(predictor_t) * num_preds );
    rc->pred = (predictor_t *)temp;
    CHECKED_MALLOC(temp , sizeof(predictor_t) );
    rc->pred_b_from_p = (predictor_t *)temp;
    for( int i = 0; i < 3; i++ )
    {
    ...
    *rc->pred_b_from_p = rc->pred[0];
    ...
    if( h->param.rc.b_stat_read )
    {
        ...
        void * temp;
        CHECKED_MALLOCZERO(temp , rc->num_entries * sizeof(ratecontrol_entry_t) );
		rc->entry = (ratecontrol_entry_t *)temp;
...
//                   &rce->s_count, &rce->direct_mode ); //TODO
			 e += sscanf( p, " in:%*d out:%*d type:%c dur:%I64d cpbdur:%I64d q:%f tex:%d mv:%d misc:%d imb:%d pmb:%d smb:%d d:%c",
					&pict_type, &rce->i_duration, &rce->i_cpb_duration, &qp, &rce->tex_bits,
					&rce->mv_bits, &rce->misc_bits, &rce->i_count, &rce->p_count,
...
static int parse_zone( x264_t *h, x264_zone_t *z, char *p )
{
    ...
    if( !*p )
        return 0;
	void *temp;
    CHECKED_MALLOC(temp , sizeof(x264_param_t) );
    z->param = (x264_param_t *)temp;
...
static int parse_zones( x264_t *h )
{
    ...
    {
        char *psz_zones, *p;
        CHECKED_MALLOC(temp , strlen( h->param.rc.psz_zones )+1 );
		psz_zones = (char *)temp;
        strcpy( psz_zones, h->param.rc.psz_zones );
        h->param.rc.i_zones = 1;
        for( p = psz_zones; *p; p++ )
            h->param.rc.i_zones += (*p == '/');
        CHECKED_MALLOC(temp , h->param.rc.i_zones * sizeof(x264_zone_t) );
		h->param.rc.zones = (x264_zone_t *)temp;
        p = psz_zones;
    ...
    if( h->param.rc.i_zones > 0 )
    {
        ...
        CHECKED_MALLOC( temp, rc->i_zones * sizeof(x264_zone_t) );
        rc->zones = (x264_zone_t *)temp;
        ...
        CHECKED_MALLOC(temp , sizeof(x264_param_t) );
		rc->zones[0].param = (x264_param_t *)temp;
...
int x264_ratecontrol_end( x264_t *h, int bits, int *filler )
{
    ...
    if( h->param.rc.b_stat_write )
    {
        ...
        if( fprintf( rc->p_stat_file_out,
                 "in:%d out:%d type:%c dur:%I64d cpbdur:%I64d q:%.2f tex:%d mv:%d misc:%d imb:%d pmb:%d smb:%d d:%c ref:",
...
static double get_qscale(x264_t *h, ratecontrol_entry_t *rce, double rate_factor, int frame_num)
{
    ...
    if( h->param.rc.b_mb_tree )
    {
        ...
        q = pow( (float)(BASE_FRAME_DURATION / CLIP_DURATION(rce->i_duration * timescale)), (float)(1 - h->param.rc.f_qcompress) );
    }
    else
        q = pow( (float)rce->blurred_complexity, (float)(1 - rcc->qcompress) );

    // avoid NaN's in the rc_eq
    if( !_finite(q) || rce->tex_bits + rce->mv_bits == 0 ) //TODO
...
static float rate_estimate_qscale( x264_t *h )
{
    float q;
    x264_ratecontrol_t *rcc = h->rc;
//    ratecontrol_entry_t UNINIT(rce);// WHY???? //TODO
...
static int vbv_pass2( x264_t *h, double all_available_bits )
{
    ...
    int adj_min, adj_max;
	void * temp;
    CHECKED_MALLOC(temp , (rcc->num_entries+1)*sizeof(double) );
	fills = (double*)temp;
...
static int init_pass2( x264_t *h )
{
	...
	void * temp;
    CHECKED_MALLOC(temp , sizeof(double)*rcc->num_entries );
	qscale = (double*)temp;
    if( filter_size > 1 )
	{   void * temp;
        CHECKED_MALLOC( temp , sizeof(double)*rcc->num_entries );
		blurred_qscale = (double *)temp;
	}
    else
