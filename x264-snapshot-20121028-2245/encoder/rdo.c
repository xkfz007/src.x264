...
#define SET_LEVEL(ndst, nsrc, l) \
    if( sizeof(trellis_level_t) == sizeof(uint32_t) )\
        ...
        trellis_level_t temp_node = {nsrc.level_idx, l };\//TODO
        level_tree[levels_used] = temp_node;\
    }\
...
//        SET_LEVEL( nodes_cur[node_ctx], nodes_prev[j], abs_level );//TODO

        if( sizeof(trellis_level_t) == sizeof(uint32_t) )
            M32( &level_tree[levels_used] ) = pack16to32( nodes_prev[j].level_idx, abs_level );
        else
		{
            trellis_level_t temp_node = { nodes_prev[j].level_idx, abs_level } ;
			level_tree[levels_used] = temp_node;
		}
        nodes_cur[node_ctx].level_idx = levels_used;
        levels_used++;	
//TODO
//#define SET_LEVEL(ndst, nsrc, l) \
//    if( sizeof(trellis_level_t) == sizeof(uint32_t) )\
...
//        SET_LEVEL( nodes_cur[j], nodes_prev[j], 0 );
        ...
        if( sizeof(trellis_level_t) == sizeof(uint32_t) )
            M32( &level_tree[levels_used] ) = pack16to32( nodes_prev[j].level_idx, 0 );
        else
        { 
            trellis_level_t temp_node = { nodes_prev[j].level_idx, 0 };
            level_tree[levels_used] = temp_node;
        }
...
#define TRELLIS_LOOP(ctx_hi)\
    ...
    {\
        ...
        {\
            ...
            {\
                /*SET_LEVEL( nodes_cur[j], nodes_cur[j], 0 );TODO\*/ \
                if( sizeof(trellis_level_t) == sizeof(uint32_t) )\
                    M32( &level_tree[levels_used] ) = pack16to32( nodes_cur[j].level_idx, 0 );\
                else\
                {\
                    trellis_level_t temp_node = { nodes_cur[j].level_idx, 0 } ;\
                    level_tree[levels_used] = temp_node;\
                }\
                nodes_cur[j].level_idx = levels_used;\
                levels_used++;\
