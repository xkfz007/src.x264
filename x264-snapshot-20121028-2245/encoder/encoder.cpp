...
static int x264_bitstream_check_buffer( x264_t *h )
{
        ...
        (h->out.bs.p_end - h->out.bs.p < max_row_size) )
    {
		...
		void * temp;
        CHECKED_MALLOC(temp , h->out.i_bitstream );
		h->out.p_bitstream = (uint8_t *)temp;
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    CHECKED_MALLOCZERO(temp_buf, sizeof(x264_t) );
    h = (x264_t *)temp_buf;
...
//	void * temp;
    CHECKED_MALLOCZERO( temp_buf , (h->frames.i_delay + 3) * sizeof(x264_frame_t *) );
    h->frames.unused[0] = (x264_frame_t **)temp_buf;
    /* Allocate room for max refs plus a few extra just in case. */
    CHECKED_MALLOCZERO(temp_buf , (h->i_thread_frames + X264_REF_MAX + 4) * sizeof(x264_frame_t *) );
    h->frames.unused[1] = (x264_frame_t **)temp_buf;
    CHECKED_MALLOCZERO(temp_buf , (h->param.i_sync_lookahead + h->param.i_bframe
                        + h->i_thread_frames + 3) * sizeof(x264_frame_t *) );
    h->frames.current = (x264_frame_t **)temp_buf;
    if( h->param.analyse.i_weighted_pred > 0 )
	{
        CHECKED_MALLOCZERO(temp_buf , h->i_thread_frames * 4 * sizeof(x264_frame_t *) );
		h->frames.blank_unused = (x264_frame_t **)temp_buf;
	}
    ...
    CHECKED_MALLOC(temp_buf , h->nal_buffer_size );
    h->nal_buffer = (uint8_t *)temp_buf;
    if( h->param.i_threads > 1 &&
        x264_threadpool_init( &h->threadpool, h->param.i_threads, (void (__cdecl *)(void *))x264_encoder_thread_init, h ) )
        goto fail;
    if( h->param.i_lookahead_threads > 1 &&
        x264_threadpool_init( &h->lookaheadpool, h->param.i_lookahead_threads, (void (__cdecl *)(void *))x264_lookahead_thread_init, h ) )
        goto fail;
    ...
    for( int i = 1; i < h->param.i_threads + !!h->param.i_sync_lookahead; i++ )
	{
        ...
        CHECKED_MALLOC( temp_buf , sizeof(x264_t) );
		h->thread[i] = (x264_t *)temp_buf;
	}
    if( h->param.i_lookahead_threads > 1 )
        for( int i = 0; i < h->param.i_lookahead_threads; i++ )
        {
            CHECKED_MALLOC(temp_buf , sizeof(x264_t) );
            h->lookahead_thread[i] = (x264_t *)temp_buf;
    ...
    for( int i = 0; i < h->param.i_threads; i++ )
    {
        ...
        CHECKED_MALLOC(temp_buf , h->out.i_bitstream );
		h->thread[i]->out.p_bitstream =(uint8_t *)temp_buf;
        /* Start each thread with room for init_nal_count NAL units; it'll realloc later if needed. */
        CHECKED_MALLOC(temp_buf, init_nal_count*sizeof(x264_nal_t) );
		h->thread[i]->out.nal = (x264_nal_t *)temp_buf;
