...
//        fps_num = round( fps_den * fps_sig ) * exponent; //TODO
		fps_num = floor( fps_den * fps_sig + 0.5 ) * exponent; 
...
//        fps_den = round( MKV_TIMEBASE_DEN / fps_sig ) / exponent;
		fps_den = floor( MKV_TIMEBASE_DEN / fps_sig  + 0.5) / exponent;
...
//            assume_fps = MKV_TIMEBASE_DEN / ( round( MKV_TIMEBASE_DEN / assume_fps_sig  ) / exponent );//TODO
			assume_fps = MKV_TIMEBASE_DEN / ( floor( MKV_TIMEBASE_DEN / assume_fps_sig +0.5) / exponent );
...
//                seq_fps = MKV_TIMEBASE_DEN / ( round( MKV_TIMEBASE_DEN / seq_fps_sig ) / exponent );
				seq_fps = MKV_TIMEBASE_DEN / ( floor( MKV_TIMEBASE_DEN / seq_fps_sig +0.5) / exponent );
...
//        x264_cli_log( "timecode", X264_LOG_INFO, "automatic timebase generation %"PRIu64"/%"PRIu64"\n", h->timebase_num, h->timebase_den );
        x264_cli_log( "timecode", X264_LOG_INFO, "automatic timebase generation %I64u/%I64u\n", h->timebase_num, h->timebase_den );
    }
...
static int open_file( char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt )
{
    ...
    if( opt->timebase )
    {
//        ret = sscanf( opt->timebase, "%"SCNu64"/%"SCNu64, &h->timebase_num, &h->timebase_den ); //TODO
        ret = sscanf( opt->timebase, "%I64u/%I64u", &h->timebase_num, &h->timebase_den ); //TODO
        if( ret == 1 )
        {
