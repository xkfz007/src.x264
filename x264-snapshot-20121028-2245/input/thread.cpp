...
{
    thread_hnd_t *h = (thread_hnd_t*)malloc( sizeof(thread_hnd_t) );
    FAIL_IF_ERR( !h || cli_input.picture_alloc( &h->pic, info->csp, info->width, info->height ),
                 "x264", "malloc failed\n" )
    h->input = cli_input;
    h->p_handle = *p_handle;
    h->next_frame = -1;
    h->next_args = (thread_input_arg_t *)malloc( sizeof(thread_input_arg_t) );
    if( !h->next_args )
        return -1;
    h->next_args->h = h;
...
{
    thread_hnd_t *h = (thread_hnd_t *)handle;
    int ret = 0;
    ...
    if( !h->frame_total || i_frame+1 < h->frame_total )
    {
        ...
        x264_threadpool_run( h->pool, (void *(__cdecl *)(void *))read_frame_thread_int, h->next_args );
    }
    else
        h->next_frame = -1;
