...
static int open_file( char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt )
{
    ...
    avs_hnd_t *h = (avs_hnd_t*)malloc( sizeof(avs_hnd_t) );
    if( !h )
        return -1;
...
static int read_frame( cli_pic_t *pic, hnd_t handle, int i_frame )
{
    ...
    avs_hnd_t *h = (avs_hnd_t*)handle;
    if( i_frame >= h->num_frames )
        return -1;
    AVS_VideoFrame *frm = (AVS_VideoFrame *)pic->opaque = h->func.avs_get_frame( h->clip, i_frame );
...
{
    avs_hnd_t *h = (avs_hnd_t*)handle;
    h->func.avs_release_video_frame((AVS_VideoFrame *)pic->opaque );
    return 0;
}
