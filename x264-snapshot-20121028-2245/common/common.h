...
/*MVD Sumf of the Left and Top Block*/
static ALWAYS_INLINE uint16_t x264_cabac_mvd_sum( uint8_t *mvdleft, uint8_t *mvdtop )
{
    ...
    amvd0 = (amvd0 > 2) + (amvd0 > 32); // 0 , 1 , 2 
    amvd1 = (amvd1 > 2) + (amvd1 > 32); // 0 , 1 , 2
    return amvd0 + (amvd1<<8); //package
}
...
typedef struct
{
    ...
    int b_direct_spatial_mv_pred;//在B图像的直接预测的模式下，用时间预测还是用空间预测。其值为1：空间预测，0：时间预测
...
struct x264_t
{
    ...
    struct
    {
        ...
        int b_have_lowres;  /* Whether 1/2 resolution luma planes are being used for slice type decision */
