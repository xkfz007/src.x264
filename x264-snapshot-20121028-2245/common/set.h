...
typedef struct
{
    ...
    int i_mb_width; // picture width in Macroblocks
    int i_mb_height;
...
typedef struct
{
    ...
    int b_cabac;	// 0 for CAVLC and 1 for CABAC

    int b_pic_order;
    int i_num_slice_groups;	//Number of Slice Groups for a Picture

    int i_num_ref_idx_l0_default_active; 	// Num of active reference For List 0 
    int i_num_ref_idx_l1_default_active;	// Num of active reference For List 1

    int b_weighted_pred;	// If used weighted prediction for P and SP slice.
    int b_weighted_bipred;	// If used weighted prediction for B and SB slice.

    int i_pic_init_qp;	// Initial QP for Luma component
    int i_pic_init_qs;	// Initial QP for Luma component in SP and SI Slice
    
	/*specifies the offset that shall be added to QPY and QSY(QP for Y Component) for addressing 
	...
	*/
    int i_chroma_qp_index_offset;  // Used specified value ??
	/*
	...
	*/
    ...
    int i_cqm_preset; 	// If Customed quantization matrix is preset
