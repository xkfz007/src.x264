...
static void mv( int x0, int y0, int16_t dmv[2], int ref, int zoom, char *col )
{
    ...
    disp_setcolor( (unsigned char *)"black" );
    disp_point( 0, x0, y0 );
    disp_setcolor( (unsigned char *)col );
}

int x264_visualize_init( x264_t *h )
{
	void * temp;
    CHECKED_MALLOC(temp , h->mb.i_mb_width * h->mb.i_mb_height * sizeof(  visualize_t) );
	h->visualize = ( visualize_t *)temp;
    return 0;
fail:
    return -1;
}
...
void x264_visualize_show( x264_t *h )
{
    ...
    int mb_xy;
    if( borders )
        disp_gray_zoom( 0, (char *)(frame - pad*stride - pad), width+2*pad, height+2*pad, stride, (const unsigned char *)"fdec", zoom );
    else
        disp_gray_zoom( 0, (char *)frame, width, height, stride, (const unsigned char *)"fdec", zoom );
