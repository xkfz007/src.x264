...
int x264_cqm_init( x264_t *h )
{
    ...
    for( int i = 0; i < 4 + num_8x8_lists; i++ )
    {
        ...
        {
			void * temp;
            CHECKED_MALLOC(temp, (QP_MAX+1)*size*sizeof(udctcoef) );
		    h->  quant4_mf[i] = (udctcoef (*)[16])temp;
            CHECKED_MALLOC(temp ,  6*size*sizeof(int) );
            h->dequant4_mf[i] = (int (*)[16])temp;
            CHECKED_MALLOC(temp , (QP_MAX+1)*size*sizeof(int) );
            h->unquant4_mf[i] = (int (*)[16])temp;
        }
        ...
        else
        {
			void * temp;
            CHECKED_MALLOC(temp , (QP_MAX+1)*size*sizeof(udctcoef) );
			h->quant4_bias[i] = (udctcoef (*)[16])temp;
            CHECKED_MALLOC(temp,  (QP_MAX+1)*size*sizeof(udctcoef) );
			h->quant4_bias0[i] = (udctcoef (*)[16])temp;
        }
    }
    ...
    x264_emms();
	void * temp;
    CHECKED_MALLOC(temp , sizeof(*h->nr_offset_emergency)*(QP_MAX-QP_MAX_SPEC) );
    h->nr_offset_emergency = (udctcoef (*)[4][64])temp;
...
{
    int i;
	// Find the pointer of the first name in string p. The start point for analysis.
    char *p = strstr( (char*)buf, name );
    if( !p )
    {
	...
	// Find the pointer of the first "INT" in string p. The end point for parser.
    ...
    if( (nextvar && p > nextvar) || i != length )//Crossing border or i<length
    {
...
/* Parse the customed quantization matrix and fill in the arrays*/
