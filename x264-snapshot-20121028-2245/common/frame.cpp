...
static x264_frame_t *x264_frame_new( x264_t *h, int b_fdec )
{
    ...
    CHECKED_MALLOCZERO(temp , sizeof(x264_frame_t) );
	frame = (x264_frame_t *)temp;
    ...
    for( int i = 0; i < h->param.i_bframe + 2; i++ )
		...
		{
            CHECKED_MALLOC( temp , i_lines/16 * sizeof(int) );
			frame->i_row_satds[i][j] = (int*)temp;
		}
    ...
    if( i_csp == X264_CSP_NV12 || i_csp == X264_CSP_NV16 )
    {
		...
		void * temp;
        CHECKED_MALLOC( temp, chroma_plane_size * sizeof(pixel) );
		frame->buffer[1] = (pixel*)temp;
        frame->plane[1] = frame->buffer[1] + frame->i_stride[1] * chroma_padv + PADH;
        if( PARAM_INTERLACED )
        {
			
            CHECKED_MALLOC( temp , chroma_plane_size * sizeof(pixel) );
			frame->buffer_fld[1] = (pixel*)temp;
    ...
    for( int p = 0; p < luma_plane_count; p++ )
    {
        ...
        if( h->param.analyse.i_subpel_refine && b_fdec )
        {
			...
			void * temp;
            CHECKED_MALLOC( temp , 4*luma_plane_size * sizeof(pixel) );
			frame->buffer[p] = (pixel*)temp;
            if( PARAM_INTERLACED )
			{
                CHECKED_MALLOC( temp , 4*luma_plane_size * sizeof(pixel) );
				frame->buffer_fld[p] = (pixel*)temp;
			}
            for( int i = 0; i < 4; i++ )
            {
        ...
        {
			void * temp;
            CHECKED_MALLOC( temp , luma_plane_size * sizeof(pixel) );
			frame->buffer[p] = (pixel*) temp;
            if( PARAM_INTERLACED )
			{
                CHECKED_MALLOC( temp , luma_plane_size * sizeof(pixel) );
    ...
    {
		void * temp;
        CHECKED_MALLOC( temp , i_mb_count * sizeof(int8_t));
		frame->mb_type = (int8_t *)temp;//TODO
        CHECKED_MALLOC( temp , i_mb_count * sizeof(uint8_t));
		frame->mb_partition = (uint8_t *) temp;
        CHECKED_MALLOC( temp , 2*16 * i_mb_count * sizeof(int16_t) );
		frame->mv[0] = (int16_t (*)[2])temp; //TODO
        CHECKED_MALLOC( temp , 2*(i_mb_count+1) * sizeof(int16_t) );
		frame->mv16x16 = (int16_t (*)[2]) temp;
        M32( frame->mv16x16[0] ) = 0;
        frame->mv16x16++;
        CHECKED_MALLOC(temp , 4 * i_mb_count * sizeof(int8_t) );
		frame->ref[0] = (int8_t *)temp;
        if( h->param.i_bframe )
        {
            CHECKED_MALLOC( temp , 2*16 * i_mb_count * sizeof(int16_t) );
			frame->mv[1] = (int16_t (*)[2])temp; 
            CHECKED_MALLOC( temp, 4 * i_mb_count * sizeof(int8_t) );
			frame->ref[1] = (int8_t *)temp;
        }
        else
        {
            frame->mv[1]  = NULL;
        ...
        CHECKED_MALLOC(temp , i_lines/16 * sizeof(int) );
		frame->i_row_bits = (int*) temp ;
        CHECKED_MALLOC(temp , i_lines/16 * sizeof(float) );
		frame->f_row_qp = (float*)temp;
        CHECKED_MALLOC(temp , i_lines/16 * sizeof(float) );
		frame->f_row_qscale = (float*)temp;
        if( h->param.analyse.i_me_method >= X264_ME_ESA )
        {
            CHECKED_MALLOC( temp ,
                            frame->i_stride[0] * (frame->i_lines[0] + 2*i_padv) * sizeof(uint16_t) << h->frames.b_have_sub8x8_esa );
			frame->buffer[3] =(pixel*)temp;
        ...
        if( PARAM_INTERLACED )
		{
            ...
            CHECKED_MALLOC( temp , i_mb_count * sizeof(uint8_t) );
			frame->field = (uint8_t *)temp;
		}
        if( h->param.analyse.b_mb_info )
		{
            CHECKED_MALLOC(temp , i_mb_count * sizeof(uint8_t) );
			 frame->effective_qp = (uint8_t *)temp;
		}
    }
    else /* fenc frame */
    {
        if( h->frames.b_have_lowres )
        {
            ...
            CHECKED_MALLOC( temp , 4 * luma_plane_size * sizeof(pixel) );
			frame->buffer_lowres[0] =(pixel*) temp;
            for( int i = 0; i < 4; i++ )
            ...
            for( int j = 0; j <= !!h->param.i_bframe; j++ )
                for( int i = 0; i <= h->param.i_bframe; i++ )
                {
					...
					frame->lowres_mvs[j][i] = (int16_t (*)[2])temp;
                    CHECKED_MALLOC(temp, h->mb.i_mb_count*sizeof(int) );
					frame->lowres_mv_costs[j][i] = (int*)temp;
                }
            CHECKED_MALLOC( temp , (i_mb_count+7) * sizeof(uint16_t) );
			frame->i_propagate_cost = (uint16_t *)temp;
            for( int j = 0; j <= h->param.i_bframe+1; j++ )
                for( int i = 0; i <= h->param.i_bframe+1; i++ )
				{
                    CHECKED_MALLOC( temp , (i_mb_count+3) * sizeof(uint16_t) );
					frame->lowres_costs[j][i] = (uint16_t *)temp;
				}
        ...
        {
            CHECKED_MALLOC( temp , h->mb.i_mb_count * sizeof(float) );
            frame->f_qp_offset = (float*)temp;
            CHECKED_MALLOC( temp , h->mb.i_mb_count * sizeof(float) );
            frame->f_qp_offset_aq = (float*)temp;
            if( h->frames.b_have_lowres )
			{
                /* shouldn't really be initialized, just silences a valgrind false-positive in x264_mbtree_propagate_cost_sse2 */
                CHECKED_MALLOCZERO( temp , (h->mb.i_mb_count+3) * sizeof(uint16_t) );
				frame->i_inv_qscale_factor = (uint16_t *)temp;
			}
        }
    }
...
int x264_sync_frame_list_init( x264_sync_frame_list_t *slist, int max_size )
{
    ...
    slist->i_size = 0;
	void * temp ;
    CHECKED_MALLOCZERO( temp , (max_size+1) * sizeof(x264_frame_t*) );
	slist->list = (x264_frame_t **)temp;
